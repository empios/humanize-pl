...
from humanize_pl.flows.docx_flow import docx_files, run_docx_flow
...
class FlowResult:
    ...
    def changed(self) -> bool:
        rows = self.payload.get("documents", self.payload.get("rows", []))
        return self.changes_applied > 0 or any(
            draft.get("inserted", True)
            for draft in self.drafted_sections + [d for row in rows for d in row.get("drafted_sections", [])]
        )
...
def _batch_warnings(payload: dict[str, Any]) -> list[str]:
    return [
        f"{item.get('name', 'pozycja')}: {warning}"
        for item in payload.get("documents", payload.get("rows", []))
...
def _build_settings(
    ...
    *,
    track: str | HumanizeTrack | None = None,
    ...
    general_options: GeneralOptions | dict | None = None,
) -> FlowSettings:
    ...
    return FlowSettings(
        ...
        **({"general_options": general_options} if general_options is not None else {}),
    )
...
def humanize(
    ...
    *,
    ...
    Args:
        ...
        track: legal or general; omitted calls infer the track from document_type.
        ...
        blueprint: Built-in blueprint ID or YAML path for structure and clause checks.
        ...
        no_rewrite: Disable editing of existing prose; drafting is separate.
        check_completeness: Check legal structure against a blueprint (default True).
        draft_missing: Explicit opt-in (default False). Write the required sections the document's category
    ...
    if source_path is not None:
        sources = docx_files(source_path) if source_path.is_dir() else [source_path]
        ensure_distinct_paths(sources, [path for path in (out_path, report) if path is not None])
    else:
        ensure_distinct_paths([], [path for path in (out_path, report) if path is not None])
    ...
    if is_path and source_path is not None and source_path.is_dir():
        ...
        if report is not None:
            report = Path(report)
            write_text_atomic(
                report, json.dumps(payload, ensure_ascii=False, indent=2), sources=docx_files(source_path)
            )
            rep_path = report
        return FlowResult(
            track=flow_settings.track.value,
            status="failed" if summary.get("failed", 0) > 0 else "ok",
            readiness_status=batch_readiness(summary),
            ...
            warnings=_batch_warnings(payload),
    ...
    if is_path and source_path is not None and source_path.suffix.lower() == ".xlsm":
        raise ValueError("XLSM z makrami nie jest obsługiwany. Użyj osobnej kopii XLSX bez makr.")
    ...
    if is_path and source_path is not None and source_path.suffix.lower() == ".docx":
        ...
        out_doc_path = Path(payload["target_docx"]) if payload.get("target_docx") else None
        return FlowResult(
            track=flow_settings.track.value,
            ...
            drafted_sections=doc.get("drafted_sections", []),
    ...
    if source_path is not None and source_path.suffix.lower() not in {".txt", ".md"}:
        raise ValueError("Obsługiwane wejścia: TXT, MD, DOCX, XLSX i folder DOCX. PDF jest raportem wyjściowym; import PDF/OCR nie jest obsługiwany.")
    text_sources = [source_path] if source_path is not None else []
    pdf_target = out_path.with_name(f"{out_path.stem}_raport.pdf") if pdf and out_path else None
    ensure_distinct_paths(text_sources, [path for path in (out_path, report, pdf_target) if path is not None])
    ...
    checkpoint("zapis wyniku")
    if out_path is not None:
        write_text_atomic(out_path, final_text, sources=text_sources)
    ...
    report_payload = {
        ...
        "settings": {
            "track": flow_settings.track.value,
            "general_options": flow_settings.general_options.to_json(),
            "mode": flow_settings.mode.value,
            "engine": flow_settings.engine.value,
            "rewrite": flow_settings.rewrite,
            "check_completeness": flow_settings.check_completeness,
            "draft_missing": flow_settings.draft_missing,
            "nli": flow_settings.nli,
        ...
        "documents": [{**outcome.to_json(), "applied_changes": outcome.applied_changes,
    ...
    if report is not None or pdf:
        ...
        if report is not None:
            write_text_atomic(
                report, json.dumps(report_payload, ensure_ascii=False, indent=2) + "\n", sources=text_sources,
            )
            rep_path = report
    ...
    return FlowResult(
        ...
        drafted_sections=outcome.drafted_sections,
        operations=outcome.operation_results(),
        nli=outcome.nli,
        tone=outcome.tone,
